...
public class ThermalLayout extends Subsystem implements Runnable {
    ...
    private final HeatValveControlled[][] eccsPspPumpValve
            = new HeatValveControlled[2][3];
    ...
    private final HeatValveControlled[][] eccsCcsPumpValve
            = new HeatValveControlled[2][3];
    ...
    private final HeatClosedFluidTank[] eccsPressureVessel
            = new HeatClosedFluidTank[2];
    private final HeatNode[] eccsPvNode = new HeatNode[2];
    private final HeatValveControlled[][] eccsPvValve
            = new HeatValveControlled[2][2];
    // Feed pump 3 to EECS lines
    private final HeatValveControlled[] eccsFPFillValve
            = new HeatValveControlled[2];
    ...
    ThermalLayout() {
        ...
        for (int idx = 0; idx < 3; idx++) {
            preheaterPiping[idx] = new HeatVolumizedFlowResistance();
    ...
    public void init() {
        ...
        for (int idx = 0; idx < 2; idx++) {
            ...
            // Connect the fuel elements - this is organized in the ReactorCore
            // class to not bloat this already bloated method too much. Each 
            // fuel channel has a connection between the distributor and the 
        ...
        for (int idx = 0; idx < 2; idx++) {
            ...
            // See Turbine section for this connection.
        }
        // Feedwater Pumps
        for (int idx = 0; idx < 2; idx++) {
            for (int jdx = 0; jdx < 2; jdx++) {
        ...
        turbineHighPressureFirst.connectBetween(turbineHighPressureMidIn,
        ...
        turbineLowPressureStage[4].connectTo(turbineLowPressureMidOut);
        // last mass goes straight into hotwell - set to PRIMARY_INNER
        // to mix it into hotwell reservor, this generates a better model.
        // for tests, the condensate will be redirected through the heat exch.
        // also to not heat up the condensate in the initial development 
        // process - TODO change back to PRIMARY_INNER as soon as condensate 
        // management works. 
        ...
        //prepare separate converters for heat in-connection...
        for (int idx = 0; idx < bubblerPoolInConverter.length; idx++) {
        ...
        for (int idx = 0; idx < 2; idx++) {
            ...
            bubblerSprinklerCoolerValve[idx].getValveElement().connectBetween(
                    coolantMainNode, bubblerSprinklerCoolerIn[idx]);
            bubblerSprinklerCooler[idx].getSecondarySide().connectBetween(
                    bubblerSprinklerCoolerIn[idx], coolantSinkNode);
        }
        ...
        for (int idx = 0; idx < 3; idx++) {
            ...
            eccsPspCoolantValve[idx].getValveElement().connectBetween(
                    coolantMainNode, eccsPspPumpCoolerIn[idx]);
            eccsPspCooler[idx].getSecondarySide().connectBetween(
                    eccsPspPumpCoolerIn[idx], coolantSinkNode);
        }
        ...
        for (int idx = 0; idx < 2; idx++) {
            mainSteamShutoffValve[idx].initCharacteristicSimple(20);
        ...
        for (int idx = 0; idx < 2; idx++) {
            ...
            blowdownValveFromLoop[idx].initCharacteristicSimple(400);
        ...
        for (int idx = 0; idx < 2; idx++) {
            feedwaterSparePumpInValve[idx].initCharacteristicSimple(200);
            feedwaterSparePumpOutValve[idx].initCharacteristicSimple(200);
        }
        ...
        for (int idx = 0; idx < 2; idx++) {
            auxCondCondensateValve[idx].initCharacteristicSimple(3e3);
            auxCondPumps[idx].initCharacteristic(5e5, 4e5, 60.0);
        }
        auxCondBypass.initCharacteristicSimple(2000);
        auxCondValveToDrain.initCharacteristicSimple(2500);
        auxCondValveToHotwell.initCharacteristicSimple(2500);
        ...
        preheaterPiping[2].setInnerThermalMass(600);
        ...
        for (int idx = 0; idx < 2; idx++) {
            ...
            turbineStartupSteamValve[idx].initCharacteristicSimple(1.5e5);
        ...
        for (int idx = 0; idx < 2; idx++) {
            bubblerSprinklerCooler[idx].getSecondarySide()
                    .setResistanceParameter(400);
            bubblerSprinklerCoolerValve[idx].initCharacteristicSimple(200);
            eccsDrainCooler[idx].getSecondarySide()
                    .setResistanceParameter(1000);
            eccsDrainCoolerValve[idx].initCharacteristicSimple(200);
        }
        for (int idx = 0; idx < 3; idx++) {
            eccsPspCooler[idx].getSecondarySide()
                    .setResistanceParameter(1000);
            eccsPspCoolantValve[idx].initCharacteristicSimple(200);
        }

        // Sprinkler pumps: Those do 200 kg/s each on 10 bar, so resistance
        // will be R = p/m_ = 10e5/200 = 5e3 Pa*s/kg
        for (int idx = 0; idx < 2; idx++) {
            bubblerSprinklerPump[idx].initCharacteristic(15e5, 10e5, 500);
        ...
        runner.submit(turbineHighPressureTapValve);
    ...
    public void run() {
        ...
        turbineShaftPower = turbineHighPressureFirst.getExcessPower()
                + turbineHighPressureSecond.getExcessPower()
...
//        }
        ...
        outputValues.setParameterValue("MainEjectors#SteamFlow",
                turbineLowPressureTapValve[3].getValveElement().getFlow());
        ...
        outputValues.setParameterValue("Preheater2#SteamFlow",
                turbineLowPressureTapValve[1].getValveElement().getFlow());
        ...
        outputValues.setParameterValue("Preheater1#DebugHeatExchangerOutTemp",
                ...
                - 273.15);
        outputValues.setParameterValue("Preheater2#DebugHeatExchangerOutTemp",
                preheater[1].getHeatNode(
                        PhasedCondenserNoMass.SECONDARY_OUT).getTemperature()
                - 273.15);
        outputValues.setParameterValue("Preheater3#DebugHeatExchangerOutTemp",
    ...
    public void handleAction(ActionCommand ac) {
        ...
        if (ac.getPropertyName().startsWith("Loop")) {
            ...
            turbineHighPressureTapValve.handleAction(ac);
